...
function tribonacci(signature, n) {
    ...
    if (n === 0) {
        ...
        final.push(final[0] + final[1] + final[2]);
        return final
    }
    let initialReduce = signature.reduce((acc, curr) => acc + curr, 0);
    final.push(initialReduce);
    for (let i = 0; i <= n - 5; i++) {
        final.push(final[i + 1] + final[i + 2] + final[i + 3])
    } return final
}
