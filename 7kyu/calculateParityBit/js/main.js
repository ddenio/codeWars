// A parity bit, or check bit, is a bit added to a string of bits to ensure that the total number of 1-bits in the string is even or odd. Parity bits are used as 
...
function checkParity(parity, bin){
    const binTotal = bin.toString().split('').reduce((total, curr) => total + Number(curr),0)
    if(parity === 'even'){
        if (binTotal % 2 === 0){
            return 0
        }return 1
