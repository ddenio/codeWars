...
// A building begins with a strength value of 1000 when first built, but this value is subject to exponential decay of 1% per year. For more info on exponential decay, 
...
function strongEnough(earthquake, age) {
    let decay = 1000*Math.pow(1-0.01, age);
    console.log(`initial decay is ${decay}`)
    let earthPower = 1;
    for (let i = 0; i<earthquake.length;i++){
        earthPower*= earthquake[i].reduce((acc,current) => acc + current, 0)
    }
    console.log(`earthpower is ${earthPower}`)
    console.log(decay-earthPower)
    if (decay - earthPower > 0){
        return 'Safe!'
    }return 'Needs Reinforcement!'
    }

console.log(strongEnough([[2,3,1],[3,1,1],[1,1,2]], 3));
console.log(strongEnough([[5,8,7],[3,3,1],[4,1,2]], 2))
console.log(strongEnough([[5,8,7],[3,3,1],[4,1,2]], 5))
console.log(strongEnough([[5,3,7],[3,3,1],[4,1,2]],2))