...
function bearDollars(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        let [hours, proximity] = arr[i];
        proximity = (proximity || '').toLowerCase().trim();
        let rate = rates.hasOwnProperty(proximity) ? rates[proximity] : 125;
        total += hours * rate;
    }return total
}

console.log(bearDollars([[10, 'Close Friend'], [3, 'Acquaintance'], [7, 'constructor'], [6, 'Friend'], [2, 'From advertisements']]))