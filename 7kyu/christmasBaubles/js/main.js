...
function baublesOnTree(baubles, branches){
  ...
  // Calculate the number of baubles per branch
  const baublesPerBranch = Math.floor(baubles / branches);
  const extraBaubles = baubles % branches;

  // Create an array to store the number of baubles on each branch
  const result = [];

  // Distribute baubles evenly to the branches
